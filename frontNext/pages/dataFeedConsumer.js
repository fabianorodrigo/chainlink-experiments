...
export default function DataFeedConsumer() {
  ...
  /**
   * getEtherPrice: gets the Ether price in USD value
   */
  const getEtherPrice = async () => {
    try {
      ...
      const _usdPrice = await consumerContract.getLatestPrice();
      //_usdPrice is a `Big Number`. We need to convert the Big Number to a string
      setPriceETH_USD(_usdPrice.div(F).toString());
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const renderButton = () => {
    ...
    return (
      <div>
        <button className={styles.button} onClick={getEtherPrice}>
          Fetch Ether Price 🪙
        </button>
      </div>
    );
        ...
        <div>
          ...
          <div className={styles.description}>
            ...
            <a
              target="_blank"
              rel="noreferrer"
              href={`https://kovan.etherscan.io/address/${DATAFEED_CONSUMER_CONTRACT_ADDRESS}`}
            >
              consumer contract
            </a>{" "}
            ...
            >
              Data Feed Agregator contract (ETH / USD)
            </a>{" "}
            to retrieve the resulting price in the last round of aggregation of
            prices sent by chainlink nodes.
          </div>
          {priceETH_USD != "0" && (
            <div className={styles.description}>
              <b>ETHER:</b> {priceETH_USD} USD
            </div>
          )}
          {renderButton()}
          <div>
